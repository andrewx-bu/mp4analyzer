...
class MP4AnalyzerMainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        # Navigation buttons
        self._playback_control.previous_button.clicked.connect(lambda: self._navigate_frame(-1))
        self._playback_control.next_button.clicked.connect(lambda: self._navigate_frame(1))
    ...
    def _load_video_file(self, file_path: str):
        ...
        try:
            ...
            if not metadata:
                ...
                QMessageBox.critical(self, "Failed to load video", "Could not extract metadata from the selected file.")
                return
    ...
    def _handle_save_snapshot(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(self, "Save Snapshot", "", "PNG Files (*.png)")
        if file_path:
            try:
                ...
                QMessageBox.critical(self, "Save Error", f"Failed to save snapshot: {str(e)}")
    ...
    def eventFilter(self, source, event):
        ...
        if event.type() == QEvent.Type.Wheel and source in (self._right_panel.video_canvas, self._right_panel.video_canvas.video_label):
