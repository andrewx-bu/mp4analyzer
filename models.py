...
class LazyVideoFrameCollection:
    ...
    def _decode_gop_frames(self, target_index: int):
        ...
        if gop_size > max_decode_frames:
            ...
            self._log(f"Large GOP ({gop_size} frames), decoding adjacent frames {decode_start}-{decode_end}")
        else:
            decode_start = gop_start
            decode_end = gop_end
            self._log(f"Decoding GOP frames {decode_start}-{decode_end} ({gop_size} frames)")
        
        # Check if we already have most frames cached
        with self._lock:
            cached_count = sum(1 for i in range(decode_start, decode_end + 1) if i in self._cache)
            if cached_count > (decode_end - decode_start + 1) * 0.8:  # 80% already cached
    ...
    def _decode_frame_range(self, start_index: int, end_index: int):
        ...
        try:
            cmd = [
                'ffmpeg', '-ss', str(start_timestamp), '-i', self._file_path,
                '-t', str(duration), '-q:v', '2', '-y', temp_pattern
            ]
            ...
            for i in range(start_index, end_index + 1):
                frame_file = os.path.join(self._temp_dir, f"frame_{i-start_index+1:04d}.png")
            ...
            if cached_frames:
                self._log(f"Cached frames {min(cached_frames)}-{max(cached_frames)} ({len(cached_frames)} frames)")
