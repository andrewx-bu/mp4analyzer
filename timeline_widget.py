...
class TimelineBarGraph(QWidget):
    ...
    FRAME_COLORS = {
        'I': QColor('red'), 'P': QColor('blue'), 'B': QColor('green'),
        '?': QColor('gray'), 'S': QColor('gray'), 'SI': QColor('gray'),
        'SP': QColor('gray'), 'BI': QColor('gray')
    }
    ...
    def set_frame_data(self, frame_data: List[FrameData]):
        ...
        total_width = len(frame_data) * (self.BAR_WIDTH + self.BAR_SPACING) if frame_data else 0
    ...
    def _center_on_selected(self):
        ...
        center_x = self._selected_frame * (self.BAR_WIDTH + self.BAR_SPACING) + self.BAR_WIDTH // 2
        viewport_width = self._scroll_area.viewport().width()
        scroll_pos = max(0, min(center_x - viewport_width // 2, self.width() - viewport_width))
    ...
    def paintEvent(self, event):
        ...
        if self._hovered_frame >= 0:
            ...
            painter.fillRect(QRect(bar_x, 0, self.BAR_WIDTH, self.height()), QColor(255, 255, 255, 40))
        ...
        if 0 <= self._selected_frame < len(self._frame_data):
            ...
            # Selection marker
            painter.fillRect(QRect(center_x - 3, 0, 5, 14), QColor('yellow'))
            
            # Selection line
            pen = QPen(QColor('yellow'), 3)
            painter.setPen(pen)
            painter.drawLine(center_x, 14, center_x, usable_height)